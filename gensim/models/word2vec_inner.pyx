...
def train_batch_LP(model, sentences, alpha, _work, _neu1, compute_loss):
    # model,
    # sentences, 词序列。问题是在于维基上，
    # alpha,
    # _work, 这是两块变量代表的内存计算空间
    # _neu1, 这是两块变量代表的内存计算空间
    # compute_loss， 不知道干啥的bool值
    ...
    cdef int window = model.window
    cdef unsigned long long modulo = 281474976710655ULL
    cdef int i, j, k,len2,idx,shuf_idx = 37
    cdef int effective_words = 0, effective_sentences = 0
    cdef int sent_idx, idx_start, idx_end
    ...
    for sent in sentences:
        ...
        for tokens in sent:
            ...
            shuf_idx = ((shuf_idx * <unsigned long long>25214903917ULL + 11) & modulo)%RATIO_Char
            # logging.info(shuf_idx)
            # if shuf_idx > 10:
            #     exit(-1)
            # # shuf_idx = np.arange(len-1,len + 1)

            if shuf_idx >= len2:
                token = tokens
            else:
                token = tokens[shuf_idx]

            #logging.info(tokens + "\t" + token)

            word = vlookup[token] if token in vlookup else None
            # logging.debug(word.index)
            if word is None:
                continue  # leaving `effective_words` unchanged = shortening the sentence = expanding the window
            if sample and word.sample_int < random_int32(&next_random):
                continue
    ...
    # precompute "reduced window" offsets in a single randint() call
    for i, item in enumerate(np.arange(effective_words)):
        reduced_windows[i] = 0
        ...
        for sent_idx in range(effective_sentences):
            ...
            for i in range(idx_start, idx_end):
                ...
                if hs:
                    fast_sentence_cbow_hs(points[i], codes[i], codelens, neu1, syn0, syn1, size, indexes, _alpha, work, i, j, k, cbow_mean, word_locks, _compute_loss, &_running_training_loss)
                if negative:
                    next_random = fast_sentence_LP(negative, cum_table, cum_table_len, codelens, neu1, syn0, syn1neg, size, indexes, _alpha, work, i, j, k, cbow_mean, next_random, word_locks, _compute_loss, &_running_training_loss)

    model.running_training_loss = _running_training_loss
    return effective_words

cdef unsigned long long fast_sentence_LP(
    const int negative, np.uint32_t *cum_table, unsigned long long cum_table_len,
