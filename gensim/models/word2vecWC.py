...
Initialize a model with e.g.::
    ...
    >>> model = Word2VecWC(sentences, size=100, window=5, min_count=5, workers=4)

Persist a model to disk with::

    >>> model.save(fname)
    >>> model = Word2VecWC.load(fname)  # you can continue training with the loaded model!
...
where "words" are actually multiword expressions, such as `new_york_times` or `financial_crisis`:
    ...
    >>> model = Word2VecWC(bigram_transformer[sentences], size=100, ...)
...
from gensim.models.word2vec_inner import train_batch_LP, train_batch_cbow
...
class Word2VecWC(utils.SaveLoad):
    """
    ...
    def __init__(self, sentences=None, size=100, alpha=0.025, window=5, min_count=5,
        ...
        logging.error('This is my Word2Vec!!')
        if FAST_VERSION == -1:
    ...
    def scale_vocab(self, min_count=None, sample=None, dry_run=False,
        ...
        if not update:
            ...
            for word, v in iteritems(self.raw_vocab):
                # LPChange i changed utils.keep_vocab_item
    ...
    def _do_train_job(self, sentences, alpha, inits):
        ...
        if self.sg:
            ...
            # logging.info('You See')
            #LPChange
            tally += train_batch_LP(self, sentences, alpha, work, neu1, self.compute_loss)
    ...
    def pushpull(self, posList, negList, sample_size = 10, alpha = 0.001):
        # print "What the hell is ", negList[0][1], "!"
        print "Train Start"


        for ww in posList:
            # print ww[0], ww[1]
            # print self.wv.index2word[self.wv.vocab[ww[0]].index]
            diff = self.wv.word_vec(ww[0]) - self.wv.word_vec(ww[1])

            self.wv.syn0[self.wv.vocab[ww[0]].index] -= alpha * diff
            self.wv.syn0[self.wv.vocab[ww[1]].index] += alpha * diff

            sampleList = random.randint(0, len(self.wv.index2word), size=sample_size)
            dist0 = np.asarray([self.wv.similarity(self.wv.index2word[ii], ww[0]) for ii in sampleList])
            mean_dist = dist0.mean()

            for i in xrange(sample_size):
                self.wv.syn0[sampleList[i]] -= (dist0[i] - mean_dist) * alpha * diff
            # print mean_dist
            # print self.similarity(ww[0], ww[1])
            #
            # self.wv.syn0[self.wv.vocab[ww[0]].index] -= alpha * diff

        print "Train POS Compelete!"

        for ww in negList:
            print ww[0], ww[1]
            print self.similarity(ww[0], ww[1])
        print "Train NEG Compelete!"
        #
        # print self.wv.syn0[5]
        # self.wv.syn0[5] += 1
        # print self.wv.syn0[5]
    ...
    def save(self, *args, **kwargs):
        ...
        super(Word2VecWC, self).save(*args, **kwargs)

    save.__doc__ = utils.SaveLoad.save.__doc__

    @classmethod
    def load(cls, *args, **kwargs):
        model = super(Word2VecWC, cls).load(*args, **kwargs)
        # update older models
    ...
    def _load_specials(self, *args, **kwargs):
        super(Word2VecWC, self)._load_specials(*args, **kwargs)
